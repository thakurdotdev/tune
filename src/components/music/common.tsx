...
export const MusicControls = memo(({ size = 'default' }: { size?: 'default' | 'large' }) => {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <Button
        ...
        className={cn('transition-all hover:scale-105', size === 'large' ? 'h-12 w-12' : 'h-9 w-9')}
      >
        <SkipForwardIcon className={size === 'large' ? 'h-6 w-6' : 'h-4 w-4'} />
      </Button>
    </div>
  );
});

export const VolumeControl = memo(({ showVolume = false }: { showVolume?: boolean }) => {
...
export const ProgressBarMusic = memo(({ isTimeVisible = false }: { isTimeVisible?: boolean }) => {
    ...
    <div className="space-y-2">
      ...
      {isTimeVisible && (
        <div className="flex justify-between">
          <p className="text-muted-foreground text-sm">{formatDuration(pos)}</p>
          <p className="text-muted-foreground text-sm">{formatDuration(duration)}</p>
        </div>
      )}
    </div>
  );
